...
class McqQuestion {
  ...
  factory McqQuestion.fromJson(Map<String, dynamic> json) {
    final rawIsVero = json['is_vero'] ?? json['correct_answer'] ?? json['answer'] ?? json['isVero'] ?? json['esito'];
    bool isVeroBool = true;
    if (rawIsVero is bool) {
      isVeroBool = rawIsVero;
    } else if (rawIsVero is String) {
      final s = rawIsVero.trim().toLowerCase();
      isVeroBool = s == 'vero' || s == '1' || s == 'true' || s == 'v' || s == 'সঠিক';
    } else if (rawIsVero is int) {
      isVeroBool = rawIsVero == 1;
    }

    final rawAudio = json['audio'] ?? json['voice'] ?? json['mp3'] ?? json['audio_file'] ?? json['audio_url'] ?? json['audio_voiceover'] ?? json['voiceover'];
    final rawImgPos = json['image_position'] ?? json['position'] ?? json['img_position'] ?? json['image_location'] ?? json['img_pos'] ?? json['pos'];

    final rawVocab = json['vocabulary'] ?? json['vocabulary_underlines'] ?? json['underlines'] ?? json['underlines_data'];
    ...
    return McqQuestion(
      ...
      italian: json['italian']?.toString() ?? json['question']?.toString() ?? json['domanda'] ?? json['question_text'] ?? json['title'] ?? '',
      bangla: json['bangla']?.toString() ?? json['bn_question']?.toString() ?? json['bn_translation']?.toString() ?? json['bangla_meaning'] ?? json['bn_meaning'] ?? json['traduzione'] ?? '',
      isVero: isVeroBool,
      image: rawImage,
